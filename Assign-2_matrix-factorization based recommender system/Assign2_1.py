...
def Get_MSE(Test,P,Q,meanR,Max,Min):
	MSE = 0.0;
	RMSE = 0.0;
...
#						for i in range(0, Mat.C):
#							aaa = Q.item((i,0));
#							print str(i)+" -> " + repr(aaa);
#						break; 
						check = abs(cur-prev);
						check = check/prev;
						if check < epsilon:
							break;
						M = Get_MSE(Test,P,Q,meanR,Max,Min);
						if M < min_MSE:
							min_MSE = M;
