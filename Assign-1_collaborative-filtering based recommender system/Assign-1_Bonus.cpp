...
/*	for Pearson method	
	int sum = 0;
	for(int a = M.row_ptr[i]; a < M.row_ptr[i+1];a++)
	{
		sum += M.val[a];
	}
	float avg_a = (float)sum/(float)M.C;

	sum = 0;
	for(int a = M.row_ptr[j]; a < M.row_ptr[j+1];a++)
	{
		sum += M.val[a];
	}
	float avg_b = (float)sum/(float)M.C;
*/


	int ni = 0, nj = 0;
	
	float both_sum = 0.0, similarity;
	
	float numerator = 0.0, lengthi = 0.0, lengthj = 0.0;
...
/*			numerator  += (M.val[ci]-avg_a) * (M.val[cj]-avg_b);
			lengthi += (M.val[ci]-avg_a) * (M.val[ci]-avg_a);
			lengthj += (M.val[cj]-avg_b) * (M.val[cj]-avg_b);
//*/
			both_sum  += M.val[cj];			// binary values
			lengthi += 1;
			lengthj += 1;

//			sqr_sum += (1-1)*(1-1);			//just a representation actually zero
...
//			sqr_sum += (1-0)*(1-0);			//just a representation actually one
...
//			sqr_sum += (0-1)*(0-1);			//just a representation actually one
...
//	if (sqr_sum != 0)
//	    similarity = 1.0/ sqrt(sqr_sum);				//euclidian distance = sqrt(sqr_sum) ==> 1/distanc = similarity

//	if (lengthi != 0 && lengthj != 0)
//	    similarity = numerator/ sqrt(lengthi)*sqrt(lengthj);	//Pearson = SUM((ui-avg)(vi-avg)) /sqrt((ui-avg)^2)sqrt((vi-avg)^2) 											==> 1/Pearson = similarity

	if (lengthi != 0 && lengthj != 0)
	    similarity = both_sum/ ((lengthi) * pow(lengthj,0.5));			
	else
	    similarity = 0.0;
